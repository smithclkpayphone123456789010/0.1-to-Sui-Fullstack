...
module admin::week_three {
    ...
    public entry fun add_nft_to_folder<T: store+key>(
        ...
        if(!dynamic_field::exists_(&folder.id, type_name)){
            dynamic_field::add(&mut folder.id, type_name, vector[nft]);
        }else{
            let subfolder = dynamic_field::borrow_mut<TypeName, vector<T>>(&mut folder.id, type_name);
            vector::push_back(subfolder, nft);
        };
    ...
    public fun check_if_nft_type_exists<T>(
        folder: &Folder
    ): bool{
        dynamic_field::exists_(&folder.id, type_name::get<T>())
    }
